...
function updateText(graphname, textname, textvalues) {
  ...
  //JPZ 11102020
  //Eduardo
  if (dayCount >= 10) {ejex2 = dayCount/2} else {ejex2 = 5}
   
  ejex=0;
  ejey=-25;
  
  if(textoptions.name == "vaccinatedText"){
  textoptions.x = ejex;
  textoptions.y = ejey;
  }
...
//parameterData[9] = {name: 'probabilityOfNoSymptoms', div: "@@@% de los contagiados son asintomáticos, luego, no son detectados por el sistema de salud.", initValue: 20, max: 100, min: 0, step: 0.1, color: '58, 100%, 50%', transform: 'parseFloat(#paraValue#)/100', requiresReset: true, runFAtEnd: ''}//parameterData[11] = {name: 'travelProbability', div: "@@@% de probabilidad de que un individuo viaje un día dado a otra comunidad.", initValue: 2, max: 10, min: 0, step: 0.01, color: '76, 100%, 50%', transform: 'parseFloat(#paraValue#)/100', requiresReset: false, runFAtEnd: ''}
...
parameterData[19] = {
  ...
  div: 
    'la segunda ola se inicia con @@@ % de infectados',
  initValue: 2.5, 
  max: 5, 
  min: 0.5, 
  step: 0.5, 
...
parameterData[20] = {
  name: 'Remov0', 
  div: 
    'la primera ola dejó @@@ % de la poblacion inmunizada',
  initValue: 15, 
  max: 30, 
  min: 0, 
  step: 1, 
...
//Eduardo
infectionStates = ['infected', 'susceptible', 'removed', 'vaccinated']
stateColors = {
  'infected':'10, 100%,70%',
  'susceptible': '195, 100%, 100%', 
  'removed':'195, 100%, 50%', 
  'noSymptoms' : '58, 100%, 50%', 
  'vaccinated':'90, 100%, 50%',
};
...
function setUpPlotGraph() {
  ...
  options.pathcolor = 'hsla(' + stateColors['vaccinated'] + ',0)';
  vaccinatedLinePath = addPath('plotG', 'vaccinatedLine', options)[0];
...
function addParticlesToBoundary(graphN, boundaryN, particleN) {
  ...
  initiallyRemovedN = boundaryData[boundaryN]['Remov0']
  ...
  for (particleIndex = 0; particleIndex < totalparticles; particleIndex++) {
	...
	//Eduardo
	else if (particleIndex < (totalparticles - (initiallyRemovedN *totalparticles/100))) {
		...
		particleCounts[graphN]['vaccinated'] = 0 ;
	}
...
function setUpSimpleCaseGraph() {
  ...
  boundaryData["simpleCaseGraphB"]["fractionInfectedInitially"] = 
    simulationParameters.fractionInfectedInitially;
  boundaryData["simpleCaseGraphB"]["Remov0"] = 
    simulationParameters.Remov0;
  
  particleCounts["simpleCaseGraphG"] = {};
  particleCounts["simpleCaseGraphG"]["vaccinated"] = 0;
  ...
  particleCountTimeLine["simpleCaseGraphG"]["vaccinated"] = [];
...
function interactions(ofBoundary) {
  ...
  for (var particleIndex in partDB) {
	...
	if (vacunado == true) 
	{ 
      if (particleData[ofBoundary][mainParticle].state == 'susceptible' ) 
	    {
		particleData[ofBoundary][mainParticle].state = 'vaccinated'
		particleData[ofBoundary][mainParticle].color = stateColors['vaccinated']
		particleCounts[graphForParticle]['susceptible'] = particleCounts[graphForParticle]['susceptible'] - 1
		particleCounts[graphForParticle]['vaccinated'] = particleCounts[graphForParticle]['vaccinated'] + 1
	  ...
	  else if (particleData[ofBoundary][mainParticle].state == 'removed' ) 
	    {
    	particleData[ofBoundary][mainParticle].state = 'vaccinated'
    	particleData[ofBoundary][mainParticle].color = stateColors['vaccinated']
...
function updatePlotGraph() {
  ...
  totalP =
    particleCounts[graphForParticle]["vaccinated"] +
    particleCounts[graphForParticle]["infected"] +
    particleCounts[graphForParticle]["susceptible"] +
    particleCounts[graphForParticle]["removed"];

  vaccinatedV = (particleCounts[graphForParticle]["vaccinated"] * 100) / totalP;
  ...
  if (particleCountTimeLine[graphForParticle]["removed"].length > 0) {
    particleCountTimeLine[graphForParticle]["vaccinated"].pop();
  ...
  particleCountTimeLine[graphForParticle]["vaccinated"].push([dayCount, 100]);
  
  particleCountTimeLine[graphForParticle]["removed"].push([
    dayCount, 
	(100 - vaccinatedV )
  ]);
  particleCountTimeLine[graphForParticle]["susceptible"].push([
    dayCount, 
	(100 - (vaccinatedV + removedV))
  ]);
  ...
  particleCountTimeLine[graphForParticle]["vaccinated"].push([dayCount, 0])
  ...
  particleCountTimeLine[graphForParticle]["vaccinated"].push([0, 0])
  ...
  updatePathPoints(
    "plotG", 
	"vaccinatedLine", 
	particleCountTimeLine[graphForParticle]["vaccinated"]
  );
  updatePathPoints(
    "plotG",
    "removedLine",
...
//Eduardo
function playVaccSim(event) {
	if (vacunacion) {
 		vacunacion = false
 		document.getElementById('Vaccbutton').value = 'sin vacunacion'
 	}
 	else {
 		vacunacion = true
 		document.getElementById('Vaccbutton').value = 'con vacunacion'
 	}
}
